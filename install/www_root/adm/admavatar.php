...
* $Id: admavatar.php,v 1.25 2005/07/28 13:29:01 hackie Exp $
*
...
function import_avatars($path)
{
	...
	if (isset($_POST['btn_update'], $_POST['edit']) && !empty($_POST['avt_img'])) {
		...
		q('UPDATE '.$tbl.'avatar SET gallery='.ssn(addslashes($avt_gal)).', img='.ssn(addslashes($_POST['avt_img'])).', descr='.ssn(addslashes($_POST['avt_descr'])).' WHERE id='.(int)$_POST['edit']);
		...
		q('INSERT INTO '.$tbl.'avatar (img, descr, gallery) VALUES ('.ssn(addslashes($_POST['avt_img'])).', '.ssn(addslashes($_POST['avt_descr'])).', '.ssn(addslashes($avt_gal)).')');
	}

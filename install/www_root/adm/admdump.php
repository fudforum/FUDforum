...
* $Id: admdump.php,v 1.79 2009/04/15 16:45:48 frank Exp $
*
...
function backup_dir($dirp, $fp, $write_func, $keep_dir, $p=0)
{
	...
	if (isset($_POST['submitted'])) {
		...
		$datadump = $_POST['path'];
		echo 'Backup process is complete!<br /><br />';
		echo '<div class="tutor">The backup file can be found at: <b>'.$datadump.'</b> (<a href="admbrowse.php?down=1&cur='. dirname($datadump) .'&dest='. basename($datadump) .'&'. __adm_rsid .'">click to download</a>). It is occupying '.filesize($_POST['path']).' bytes.</div><br />';
	} else {

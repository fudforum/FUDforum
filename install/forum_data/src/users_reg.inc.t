...
class fud_user_reg extends fud_user
{
	...
	{	
		...
		if ($GLOBALS['MODERATE_USER_REGS'] == 'Y') {
			$this->users_opt = $this->users_opt ^ 2097152;
		}
...
function usr_email_unconfirm($id)
{
	...
	q("UPDATE {SQL_TABLE_PREFIX}users SET email_conf=email_conf &~ 131072, conf_key='".$conf_key."' WHERE id=".$id);
	db_unlock();

...
* $Id: ratethread.php.t,v 1.13 2004/11/24 19:53:36 hackie Exp $
*
...
/*{POST_HTML_PHP}*/
	...
	if (isset($_POST['rate_thread_id'], $_POST['sel_vote'])) {
		...
		if (!q_singleval('SELECT t.id
				...
				WHERE t.id='.$th.($is_a ? '' : ' AND (m.id IS NOT NULL OR ((CASE WHEN g1.id IS NULL THEN g2.group_cache_opt ELSE g1.group_cache_opt END) & 1024) > 0)')  . ' LIMIT 1')) {
			std_error('access');
		}

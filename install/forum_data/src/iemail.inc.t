...
function send_notifications($to, $msg_id, $thr_subject, $poster_login, $id_type, $id, $frm_name='')
{
	...
	$goto_url['icq'] = "javascript:window.location='".$icq."rview.php?goto=".$msg_id."';";
	$goto_url['email'] = $GLOBALS['WWW_ROOT'].'rview.php?goto='.$msg_id;
	...
	if( $id_type == 'thr' ) {
		...
		if( !$body_email ) {
			$unsub_url['email'] = $GLOBALS['WWW_ROOT'].'rview.php?th='.$id.'&notify=1&opt=off';
			$body_email = '{TEMPLATE: iemail_thr_bodyemail}';
		}	
		
		$body_icq = '{TEMPLATE: iemail_thr_bodyicq}';
		$unsub_url['icq'] = "javascript:window.location='".$icq."rview.php?th=".$id."&notify=1&opt=off';";
	}
	else if ( $id_type == 'frm' ) {
		...
		$unsub_url['icq'] = "javascript:window.location='".$icq."thread.php?unsub=1&frm_id=".$id."';";
		$body_icq = '{TEMPLATE: iemail_frm_bodyicq}';
		
		if( !$body_email ) {
			$unsub_url['email'] = $GLOBALS['WWW_ROOT'].'rview.php?thread.php?unsub=1&frm_id='.$id;

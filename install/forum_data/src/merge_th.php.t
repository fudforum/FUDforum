...
* $Id: merge_th.php.t,v 1.31 2005/08/11 01:26:13 hackie Exp $
*
...
/*{POST_HTML_PHP}*/
	...
	$c = uq('SELECT f.id, f.name
			...
			'.($is_a ? '' : ' WHERE mm.id IS NOT NULL OR '.(_uid ? '(COALESCE(g2.group_cache_opt, g1.group_cache_opt) & 2) > 0' : '(g1.group_cache_opt & 2) > 0')).'
			ORDER BY v.id');
	$vl = $kl = '';
	while ($r = db_rowarr($c)) {
		$vl .= $r[0] . "\n";

...
* $Id$
*
* This program is free software; you can redistribute it and/or modify ita
...
function ses_get($id=0)
{
	...
	if (!$id) {
		...
		$cookie = $_COOKIE[$GLOBALS['COOKIE_NAME']] ?? null;
		if ($cookie) {
			/* Have cookie? */
			$q_opt = 's.ses_id='. _esc($cookie);
			...
			if ($spider_session) {
				...
				if ($id = db_li('INSERT INTO {SQL_TABLE_PREFIX}ses (ses_id, time_sec, sys_id, ip_addr, useragent, user_id) VALUES (\''. $spider['botname'] .'\', '. __request_timestamp__ .', '. _esc(ses_make_sysid()) .', '. _esc($my_ip) .', '. _esc(substr($HTTP_USER_AGENT, 0, 64)) .', '. $spider['user_id'] .')', $ef, 1)) {
					$q_opt = 's.id='. $id;
				} else {
...
function ses_anon_make()
{
	...
	} while (!($id = db_li('INSERT INTO {SQL_TABLE_PREFIX}ses (ses_id, time_sec, sys_id, ip_addr, useragent, user_id) VALUES (\''. $ses_id .'\', '. __request_timestamp__ .', '. _esc(ses_make_sysid()) .', '. _esc(get_ip()) .', '. _esc($HTTP_USER_AGENT) .', '. $uid .')', $ef, 1)));
...
function ses_update_status($ses_id, $action=null, $forum_id=0, $ret='')
{
	...
	q('UPDATE {SQL_TABLE_PREFIX}ses SET sys_id=\''. $sys_id .'\', forum_id='. (int)$forum_id .', time_sec='. __request_timestamp__ .', action='. ($action ? _esc($action) : 'NULL') .', returnto='. (!is_int($ret) ? (isset($_SERVER['QUERY_STRING']) ? _esc($_SERVER['QUERY_STRING']) : 'NULL') : 'returnto') .' WHERE id='. $ses_id);
}
...
function ses_delete($ses_id)
{
	...
	q('DELETE FROM {SQL_TABLE_PREFIX}ses WHERE id='. (int)$ses_id);

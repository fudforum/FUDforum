...
function tmpl_create_pager($start, $count, $total, $arg, $suf='', $append=1, $js_pager=FALSE)
{
	...
	if ( !$count ) $count = $GLOBALS["POSTS_PER_PAGE"];
	...
	if ( !$js_pager )
		$page_pager_data = substr($page_pager_data,0,strlen('{TEMPLATE: page_separator}')*-1);
	else 
		$page_pager_data = substr($page_pager_data,0,strlen('{TEMPLATE: page_separator_js}')*-1);

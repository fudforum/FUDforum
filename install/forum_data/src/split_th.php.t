...
*
*   $Id: split_th.php.t,v 1.26 2003/10/03 23:59:46 hackie Exp $
****************************************************************************
...
/*{POST_HTML_PHP}*/
	...
	$c = uq('SELECT f.id, f.name
			...
			'.($usr->users_opt & 1048576 ? '' : ' WHERE mm.id IS NOT NULL OR (CASE WHEN g2.id IS NULL THEN g1.group_cache_opt ELSE g2.group_cache_opt END) & 4').'
			ORDER BY v.id');
	$vl = $kl = '';
	while ($r = db_rowarr($c)) {
		$vl .= $r[0] . "\n";

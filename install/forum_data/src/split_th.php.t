...
* $Id: split_th.php.t,v 1.52 2005/08/26 13:57:47 hackie Exp $
*
...
function th_frm_last_post_id($id, $th)
{
	...
	if ($forum && !empty($_POST['new_title']) && is_string($_POST['new_title']) && !empty($_POST['sel_th']) && is_array($_POST['sel_th'])) {
		...
		if ($mc != ($data->replies + 1)) { /* check that we need to move the entire thread */
			if ($forum != $data->forum_id) {
				$lk_pfx = '{SQL_TABLE_PREFIX}tv_'.$forum.' WRITE,{SQL_TABLE_PREFIX}thread t,{SQL_TABLE_PREFIX}msg m WRITE';
			} else {
				$lk_pfx = '';
			}
	...
	$c = uq('SELECT f.id, f.name
			...
			'.(_uid ? 'COALESCE(g2.group_cache_opt, g1.group_cache_opt)' : 'g1.group_cache_opt'). ' & 4) > 0').'
			ORDER BY v.id');
	$vl = $kl = '';
	while ($r = db_rowarr($c)) {
		$vl .= $r[0] . "\n";

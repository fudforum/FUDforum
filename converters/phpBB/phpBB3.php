...
* $Id: phpBB3.php,v 1.43 2006/11/27 23:58:57 frank Exp $
*
...
***************************************************************************/
	...
	/* PHPBB 3 (3.0.X) - FUDforum Conversion script - Brief Instructions */
	...
	/* Specify the FULL path to the phpBB3 config.php file */
	$PHPBB_CFG = "/lamp/apache2/htdocs/phpBB3/config.php";
...
function phpbb_decode_ip($int_ip)
{
	...
	if ($gl === FALSE) {
		exit("Unable to open phpBB3 configuration at '{$PHPBB_CFG}'.\n");
	}

	if (!($ib = mysql_connect($dbhost, $dbuser, $dbpasswd))) {
		exit("Failed to connect to database containing phpBB3 settings using MySQL information inside '{$PHPBB_CFG}'.\n");
	}
	define('phpsql', $ib);
	...
	$board_config = array();
	print_msg('Reading phpBB3 config');
	$r = bbq("SELECT * FROM {$bb2}config");
	while (list($k,$v) = db_rowarr($r)) {
		$board_config[$k] = $v;
	}
	unset($r);
	print_msg('Done: Reading phpBB3 config');
...
/* Import phpBB smilies */
	...
	while ($obj = db_rowobj($r)) {
		if (!q_singleval("SELECT id FROM ".$DBHOST_TBL_PREFIX."smiley WHERE img='".$obj->smiley_url."'")) {
			if (!copy($PHPBB_INSTALL_ROOT.$board_config['smilies_path'].'/'.$obj->smiley_url, $IMG_ROOT_DISK.'smiley_icons/'.$obj->smiley_url)) {
				print_msg("Coulnd't copy smiley image (".$PHPBB_INSTALL_ROOT.$board_config['smilies_path'].$obj->smiley_url.") to (".$IMG_ROOT_DISK.'smiley_icons/'.$obj->smiley_url.")");
				exit;
			}
			q("INSERT INTO ".$DBHOST_TBL_PREFIX."smiley (img,code,descr,vieworder) VALUES('{$obj->smiley_url}','".addslashes($obj->code)."','".addslashes($obj->emotion)."', '{$i}')");
		} else {
			q("UPDATE ".$DBHOST_TBL_PREFIX."smiley SET code=".__FUD_SQL_CONCAT__."(code, '~".addslashes($obj->code)."') WHERE img='{$obj->smiley_url}'");
		}
	}
	umask($old_umask);
	unset($r);
...
/* Import phpBB3 users */
	...
	while ($obj = db_rowobj($r)) {
		...
		if ($obj->user_type == USER_FOUNDER) {
			$users_opt |= 1048576;
		// } else if ($obj->user_type == MOD) {
		//	$users_opt |= 524288;
		}

		if ($obj->user_allow_viewemail) {
			$users_opt |= 1;
		}
		...
		if ($obj->user_type != USER_INACTIVE) {	
			$users_opt |= 131072;	// email_conf
		}
...
/* Import phpBB3 Categories */
	...
	$r = bbq("select * from {$bb2}forums WHERE forum_type = 0 ORDER BY parent_id");
	print_msg('Importing Categories '.db_count($r));
	$i = 1;
	while( $obj = db_rowobj($r) ) {
		q("INSERT INTO ".$DBHOST_TBL_PREFIX."cat (id, name, view_order, cat_opt) VALUES(".(int)$obj->forum_id.",'".addslashes($obj->forum_name)."',".$i++.", 3)");
	}
	unset($r);
	print_msg('Finished Importing Categories');

/* Import phpBB3 Forums */
...
$forum_map = array();
	...
	$r = bbq("select * from {$bb2}forums WHERE forum_type = 1 ORDER BY parent_id");
	print_msg('Importing Forums '.db_count($r));
	while ($obj = db_rowobj($r)) {
		$_POST['frm_cat_id'] = $obj->parent_id;
...
/* Import phpBB messages */
	...
	$r = bbq("SELECT t.forum_id, t.topic_title, p.* FROM {$bb2}posts p INNER JOIN {$bb2}topics t ON t.topic_id=p.topic_id");
...
/* FRANK
	...
	while ($obj = db_rowobj($r)) {
		...
		$r2 = bbq("SELECT * FROM {$bb2}pole_votes");
...
/* Import phpBB private messages */
	...
	$r = bbq("SELECT p.*, u.username FROM {$bb2}privmsgs p INNER JOIN {$bb2}users u ON u.user_id=p.to_address");
	...
	while ($obj = db_rowobj($r)) {
		...
		list($off, $len) = write_pmsg_body(bbcode2fudcode($obj->message_text));
		...
		if ($obj->to_address == 1) {
			$obj->to_address = $hack_id;
		}
		if ($obj->author_id == 1) {
			$obj->author_id = $hack_id;
		}
		...
		q("INSERT INTO ".$DBHOST_TBL_PREFIX."pmsg 
			...
			VALUES(
				".(int)$obj->author_id.",
				".(int)$obj->to_address.",
				'".phpbb_decode_ip($obj->author_ip)."',
				".(int)$obj->message_time.",
				".$read_stamp.",
				".$folder.",
				'".addslashes($obj->message_subject)."',
				".$pmsg_opt.",
				".$off.",
				".$len.",
				'".addslashes($obj->username)."')"
...
/* Import phpBB settings */
	...
	print_msg("\n\nConversion of phpBB3 to FUDforum2 has been completed\n Time Taken: ".$time_taken."\n");
